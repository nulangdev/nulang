...
func createRegExpObject(re *RegExp) *object.ObjectMap {
	...
	obj.Set("test", &object.Builtin{Name: "test", Fn: func(args ...object.Object) object.Object {
		...
		return nativeBoolToBooleanObject(re.Regexp.MatchString(str))
	}})
	...
	obj.Set("exec", &object.Builtin{Name: "exec", Fn: func(args ...object.Object) object.Object {
		...
		match := re.Regexp.FindStringSubmatch(str)
		if match == nil {
			return NULL
		}
	...
	obj.Set("match", &object.Builtin{Name: "match", Fn: func(args ...object.Object) object.Object {
		...
		var matches []string
		if re.Global {
			matches = re.Regexp.FindAllString(str, -1)
		} else {
			match := re.Regexp.FindString(str)
			if match != "" {
	...
	obj.Set("replace", &object.Builtin{Name: "replace", Fn: func(args ...object.Object) object.Object {
		...
		var result string
		if re.Global {
			result = re.Regexp.ReplaceAllString(str, replacement)
		} else {
			result = re.Regexp.ReplaceAllStringFunc(str, func(match string) string {
				// Only replace first match
				return replacement
			})
			// Actually need to only replace first
			loc := re.Regexp.FindStringIndex(str)
			if loc != nil {
	...
	obj.Set("split", &object.Builtin{Name: "split", Fn: func(args ...object.Object) object.Object {
		...
		parts := re.Regexp.Split(str, limit)
	...
	case "match":
		return &object.Builtin{Name: "match", Fn: func(args ...object.Object) object.Object {
			...
			case *object.String:
				...
				compiled, err := regexp.Compile(arg.Value)
				if err != nil {
					return NULL
				}
			...
			var matches []string
			if re.Global {
				matches = re.Regexp.FindAllString(str.Value, -1)
			} else {
				match := re.Regexp.FindString(str.Value)
				if match != "" {
					matches = []string{match}
				}
			}
	...
	case "replace":
		return &object.Builtin{Name: "replace", Fn: func(args ...object.Object) object.Object {
			...
			case *RegExp:
				var result string
				if arg.Global {
					result = arg.Regexp.ReplaceAllString(str.Value, replacement)
				} else {
					loc := arg.Regexp.FindStringIndex(str.Value)
					if loc != nil {
						result = str.Value[:loc[0]] + replacement + str.Value[loc[1]:]
					} else {
						result = str.Value
					}
				}
			...
			case *object.ObjectMap:
				if reObj, ok := arg.Get("_regexp"); ok {
					if re, ok := reObj.(*RegExp); ok {
						var result string
						if re.Global {
							result = re.Regexp.ReplaceAllString(str.Value, replacement)
						} else {
							loc := re.Regexp.FindStringIndex(str.Value)
							if loc != nil {
								result = str.Value[:loc[0]] + replacement + str.Value[loc[1]:]
							} else {
								result = str.Value
